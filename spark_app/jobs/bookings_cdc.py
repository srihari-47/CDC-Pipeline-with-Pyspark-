...
from spark_app.config.schema import BOOKINGS_DEBEZIUM_SCHEMA
from spark_app.utils.CDCBaseClass import CDCBaseClass
import logging
import sys


class BookingCDCProcessor(CDCBaseClass):
    """
    Processes Change Data Capture (CDC) streams for Booking-related data.

    This class inherits from CDCBaseClass and implements specific data transformation
    logic for processing Booking data streams. The transformation involves filtering out
    ...
    def transform_data(self, stream):
        try:
            ...
            df_ = stream.select(from_json(col('value').cast('String'), BOOKINGS_DEBEZIUM_SCHEMA).alias('data'))
            ...
            df_ = (df_.filter(col('data.op').isNotNull())
                   .select(col("data.op").alias("DB_OP"),
                           col('data.after.booking_id').alias('BOOKING_ID'),
                           col('data.after.customer_id').alias('CUSTOMER_ID'),
                           col('data.after.flight_id').alias('FLIGHT_ID'),
                           from_unixtime(col('data.after.created_at') /micro_to_sec).alias('CREATED_AT_UTC'),
                           from_unixtime(col('data.after.updated_at')).alias('LAST_UPDATED_AT_UTC'),
                           col('data.before').alias('BOOKING_DATA_BEFORE_UPD'),
...
def booking_cdc(spark):
    """
    Processes CDC (Change Data Capture) for booking data.

    This function creates an instance of the BookingCDCProcessor class, passing
    the provided Spark session. It then initiates the CDC process for the 'bookings'
    dataset. This function is designed to handle CDC processing logic, enabling
    the system to track and manage incremental changes in Booking data.

    :param spark: Spark session used for executing the process.

    """
    cdc_processor = BookingCDCProcessor(spark,'bookings')
    cdc_processor.run()
...
if __name__ == '__main__':
    ...
    with start_spark_session() as spark_session:
        try:
            logger.info("Starting the CDC job for BOOKINGS table")
            booking_cdc(spark_session)
            logger.info("Completed the CDC job for BOOKINGS table")
        except KeyboardInterrupt:
            logger.info("User terminated the CDC job for BOOKINGS table execution. Exiting the application..")
            exit_code = 1
        except Exception as e:
            logger.error(f"Error occurred while running the CDC job for BOOKINGS table: {e}")
            exit_code = 2
    logging.shutdown()
    sys.exit(exit_code)
